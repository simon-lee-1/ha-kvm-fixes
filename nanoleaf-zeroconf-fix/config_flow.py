"""Config flow for Nanoleaf integration — patched to not update host from Zeroconf.

Problem: Nanoleaf advertises IPv6 ULA via mDNS. The built-in integration updates
the config entry host to this IPv6 address, which is unreachable through KVM NAT.
This happens within 10 seconds of every HA boot.

Fix: Override _async_discovery_handler to call _abort_if_unique_id_configured()
WITHOUT passing {CONF_HOST: host}, so existing entries keep their IPv4 address.
"""
...
class NanoleafConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_zeroconf(
        ...
        """Handle Nanoleaf Zeroconf discovery — DO NOT update host."""
        _LOGGER.debug(
            "Zeroconf discovered: %s (host update suppressed)", discovery_info
        )
    ...
    async def async_step_homekit(
        ...
        _LOGGER.debug(
            "Homekit discovered: %s (host update suppressed)", discovery_info
        )
        return await self._async_homekit_zeroconf_discovery_handler(discovery_info)
    ...
    async def _async_discovery_handler(
        ...
        PATCHED: Does NOT pass {CONF_HOST: host} to _abort_if_unique_id_configured().
        The original code updates the config entry host on every discovery event,
        which overwrites a working IPv4 with an unreachable IPv6 ULA when behind NAT.
        """
        await self.async_set_unique_id(name)
        # PATCH: abort without updating host
